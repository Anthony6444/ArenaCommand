...
$(document).on("click", ".robot-image-list-item", function (event) {
  ...
  $("#current-id").text(
    $(cur_target).children(".card-body").children("#id").text()
  );
...
$("#upload-image-button").on("click", function (event) {
  ...
  } else if ($("#form-image-file").get(0).files.length != 0) {
    // console.log($('#form-image-file').get(0).files.length)
    var imageFile = $("#form-image-file").get(0).files[0];
  ...
  function startup() {
    ...
    deviceselector.addEventListener("change", (ev) => {
      ...
      navigator.mediaDevices
        .getUserMedia({
          video: ev.target.value ? { deviceId: ev.target.value } : undefined,
          audio: false,
        })
        .then((stream) => {
    ...
    navigator.mediaDevices
      .getUserMedia({
        video: deviceselector.children[0].value
          ? { deviceId: deviceselector.children[0].value }
          : undefined,
        audio: false,
      })
      .then((stream) => {
