$(document).on("mouseover mouseout", "tr", function (event) {
    if (event.type == "mouseover") {
        ...
        $($(cur_target).children(".rle-edit-button-container").children("button.edit-button")).removeClass("invisible");
        ...
        $($(cur_target).children(".rle-edit-button-container").children("button.edit-button")).addClass("invisible");
    }
})
$(document).on("click", ".edit-button", function (event) {
    ...
    $("#form-robot-name").val($("#" + tr.id + "-robot-name").text().trim());
    $("#form-title-robot-name").text($("#" + tr.id + "-robot-name").text().trim());
...
function populateRobotEditableList() {
    $.get("/static/rle-boilerplate.html", success = function (result) {
        rle_boilerplate = result;
        $.get("/api/v1/list/all", success = function (result) {
            robot_editable_list = "";
            for (i = 0; i < result.length; i++) {
                this_robot = $("<div>").html(rle_boilerplate
                ...
                if (result[i]["imagestatus"] == "ok") {
                    this_robot.children("tr").children("td").children("#rle-image-ok").removeClass("hidden")
                } else if (result[i]["imagestatus"] == "warn") {
                    this_robot.children("tr").children("td").children("#rle-image-warn").removeClass("hidden")
                } else if (result[i]["imagestatus"] == "error") {
                    this_robot.children("tr").children("td").children("#rle-image-error").removeClass("hidden")
                } else {

                }
                if (result[i]["existsinchallonge"]) {
                    this_robot.children("tr").children("td").children("#rle-ch-ok").removeClass("hidden")
                } else {
                    this_robot.children("tr").children("td").children("#rle-ch-error").removeClass("hidden")
                }
        ...
        fileReader.onload = function () {
            ...
            $(".csv-header-select").each(function (index, element) {
                ...
                csvdata[0].forEach(row => {
                    new_option = $("<option>")
                    new_option.val(row).text(row.length < 50 ? row : (row.slice(0, 50) + "..."))
